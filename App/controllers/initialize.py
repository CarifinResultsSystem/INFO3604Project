...
def initialize():
    ...
    institution = create_institution("Scotiabank", "Port of Spain")
    event = create_event("1 Lap Savannah", "2026-06-15", "09:00","Queens Park Savannah", season.seasonID)
    create_participant(str(uuid4()), "Dave", "Darren", "Male", "2000-01-01", "San Fernando", institution.institutionID, [event[0].eventID])
    ...
    rule1 = create_team_rule(event[0].eventID, season.seasonID, "Participation/Attendance", "Mr. Carifin Representative", 10)
    rule2 = create_team_rule(event[0].eventID, season.seasonID, "Participation/Attendance", "Mrs. Carifin Representative", 10)
    rule3 = create_team_rule(event[0].eventID, season.seasonID, "Participation/Attendance", "Team Uniformity", 10)